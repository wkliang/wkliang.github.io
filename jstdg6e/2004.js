...
// Since the handlers all display status message this way, they return false
// to cancel the event and prevent the browser from displaying its onw status.
function status(msg) {
...
// The checking event always fired first when this process begins.
...
// If the applcation is not already cached, or if the manifest has changed,
...
// fires the cache event when the download is complete.
...
// If the browser is offline and manifest cannot be checked, an "error"
...
// Subsequent loads are down from the network rather than from the cache.
