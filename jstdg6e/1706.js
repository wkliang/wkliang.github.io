 ...
 * 	<span id="zipwarn" style:"color:red;visibility:hidden">Digit only</span>
 *
 * This module is purely unobtrusive: it dose not define any symbols in 
 * the global namespace.
 */
whenReady(function() {	// Run this function when the document is loaded
    // Find all <input> elements
    var inputelts = documents.getElementsByTagName("input");
	...
	// textinput (lowercase) is the version in DOM Level 3 Events draft.
	if (elt.addEventListener) {
	    elt.addEventListener("keypress", filter, false);
	    elt.addEventListener("textInput", filter, false);
	    elt.addEventListener("textinput", filter, false);
	}
	else {	// textinput not supported versions of IE w/o addEventListner()
