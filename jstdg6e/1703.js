...
function enclose(content, framewidth, frameheight, contentX, contentY) {
    ...
    frame.onmousewheel = wheelHandler;	// Most current browser
    if (isFirefox)
	...
	// If future browsers fire both "wheel" and "mouswheel" for the same
	...
	var deltaY = e.deltaY*-30 ||	// wheel event
		e.wheelDataY/4	  ||	// mousewheel event in Webkit
	(e.wheelDeltaY === undefined &&	// if there is no 2D property then
		 e.wheelDelta/4)  ||	// use the 1D wheel property
		e.detail*-10      ||	// Firefox DOMMouseScroll event
				0;	// property no defiend

	// Most browsers generate one event with delta 120 per mousewheel click.
	// on Macs, however, the mousewheels seem to be velocity-sensitive and
	// the delta values are often larger multiples of 120, at
	// least with the Apple Mouse. Use the browser-testing to defeat this.
	if (isMacWebkit) {
	    deltaX /= 30;
	    deltaY /= 30;
	}
	...
	if (isFirefox && e.type != "DOMMouseScroll") {
	    console.log("Firefox no longer need DOMMouseScroll");
